...
__global__ void reduction_min_first(PseudoJet *jets, double *distances, double *distances_out, int *indices,
                                    int const distances_array_size, int const num_particles)
{
...
__global__ void reduction_min_second(PseudoJet *jets, double *distances, double *distances_out, int *indices,
                              int const distances_array_size, int const num_particles)
{
    extern __shared__ double sdata[];
    double *s_distances = sdata;
...
int main()
{
    ...
    for (int n = NUM_PARTICLES; n > 0; n--)
    {
        ...
        reduction_min_second<<<1, num_blocks,
